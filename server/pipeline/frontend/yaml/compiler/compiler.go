...
func (c *Compiler) RestoreCache(conf *yaml.Config, ir *backend.Config) {
	if c.local || conf.Cache.Key == "" || c.cacher == nil {
		return
	}

	// container := c.cacher.Restore(c.metadata.Repo.Name, c.metadata.Curr.Commit.Branch, conf.Cache)
	container := c.cacher.Restore(c.metadata.Repo.Name, &conf.Cache)
...
func (c *Compiler) SaveCache(conf *yaml.Config, ir *backend.Config) {
	if c.local || conf.Cache.Key == "" || c.metadata.Curr.Event != "push" || c.cacher == nil {
		return
	}

	// container := c.cacher.Save(c.metadata.Repo.Name, c.metadata.Curr.Commit.Branch, conf.Cache)
	container := c.cacher.Save(c.metadata.Repo.Name, &conf.Cache)
	name := fmt.Sprintf("%s_save_cache", c.prefix)
	step := c.createProcess(name, container, "cache")

	stage := new(backend.Stage)
	stage.Name = name
	stage.Alias = "save_cache"
	stage.Steps = append(stage.Steps, step)
