...
func (m *Metadata) EnvironDrone() map[string]string {
	...
	params := map[string]string{
		...
		"CI_REPO_NAME":            name,
		"CI_REPO_LINK":            m.Repo.Link,
		"CI_REPO_BRANCH":          m.Repo.Branch,
		"CI_REPO_PRIVATE":         fmt.Sprintf("%v", m.Repo.Private),
		...
		"CI_COMMIT_AUTHOR_EMAIL":  m.Curr.Commit.Author.Email,
		"CI_COMMIT_AUTHOR_AVATAR": m.Curr.Commit.Author.Avatar,
		"CI_BUILD_NUMBER":         fmt.Sprintf("%d", m.Curr.Number),
		"CI_PARENT_BUILD_NUMBER":  fmt.Sprintf("%d", m.Curr.Parent),
		"CI_BUILD_EVENT":          m.Curr.Event,
		"CI_BUILD_LINK":           fmt.Sprintf("%s/%s/%d", m.Sys.Link, m.Repo.Name, m.Curr.Number),
		"CI_BUILD_CREATED":        fmt.Sprintf("%d", m.Curr.Created),
		"CI_BUILD_STARTED":        fmt.Sprintf("%d", m.Curr.Started),
		"CI_BUILD_FINISHED":       fmt.Sprintf("%d", m.Curr.Finished),
		"CI_JOB_NUMBER":           fmt.Sprintf("%d", m.Job.Number),
		"CI_JOB_STARTED":          fmt.Sprintf("%d", m.Curr.Started), // ISSUE: no job started
		"CI_BRANCH":               m.Curr.Commit.Branch,
		"CI_COMMIT":               m.Curr.Commit.Sha,
		"CI_VERSION":              m.Sys.Version,
		"CI_DEPLOY_TO":            m.Curr.Target,
		"CI_PREV_BUILD_STATUS":    m.Prev.Status,
		"CI_PREV_BUILD_NUMBER":    fmt.Sprintf("%v", m.Prev.Number),
		"CI_PREV_COMMIT_SHA":      m.Prev.Commit.Sha,
        // drone
		"DRONE":                      "true",
