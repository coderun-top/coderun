...
func (l *Linter) lint(containers []*yaml.Container, block uint8) error {
	duplicateCName := make(map[string]bool)
	for _, container := range containers {
		cName := container.Name
		if _, value := duplicateCName[cName]; !value {
			duplicateCName[cName] = true
		} else {
			return fmt.Errorf("Cannot configure duplicate container name: %s.", cName)
		}
