...
import (
	...
	"github.com/coderun-top/coderun/src/remote/coding/internal"
...
func (c *Coding) Login(res http.ResponseWriter, req *http.Request) (*model.User, error) {
	...
	t := 1
	user, err := c.newClientToken(token.AccessToken, token.RefreshToken).GetCurrentUser(t)
	if err != nil {
		return nil, err
	}
...
func (c *Coding) Auth(token, secret string) (string, error) {
	...
	user, err := c.newClientToken(token, secret).GetCurrentUser(t)
	if err != nil {
		return "", err
	}
...
func (c *Coding) Repo(u *model.User, owner, repo string) (*model.Repo, error) {
	...
	nn := strings.Split(repo, "/")
	t := 1
	if u.Oauth {
		t = 2
	}
	project, err := c.newClient(u).GetProject(nn[0], nn[1], t)
	if err != nil {
		return nil, err
	}
	depot, err := c.newClient(u).GetDepot(nn[0], nn[1], t)
	if err != nil {
		return nil, err
	}
	return &model.Repo{
...
func (c *Coding) User(token string, t int) (*model.User, error) {
	user, err := c.newClientToken(token, "").GetCurrentUser(t)
	if err != nil {
		return nil, err
	}
...
func (c *Coding) AccessToken(token string) (string, error) {
	return "", nil
}

func (c *Coding) Branches(u *model.User, owner, name string) ([]*model.Branch, error) {
	...
	branches, err := c.newClient(u).GetBranches(nn[0], nn[1], t)
	if err != nil {
		return nil, err
	}
...
func (c *Coding) Branch(u *model.User, owner, name, branchName string) (*model.Branch, error) {
	...
	branch, err := c.newClient(u).GetBranch(nn[0], nn[1], t, branchName)
	if err != nil {
		return nil, err
	}
...
func (c *Coding) Repos(u *model.User) ([]*model.Repo, error) {
	...
	projectList, err := c.newClient(u).GetProjectList(t)
	if err != nil {
		return nil, err
	}
	...
	for _, project := range projectList {
		depot, err := c.newClient(u).GetDepot(project.Owner, project.Name, t)
		if err != nil {
			return nil, err
		}
		repo := &model.Repo{
...
func (c *Coding) Perm(u *model.User, owner, repo string) (*model.Perm, error) {
	...
	project, err := c.newClient(u).GetProject(owner, repo, t)
	if err != nil {
		return nil, err
	}
...
func (c *Coding) File(u *model.User, r *model.Repo, b *model.Build, f string) ([]byte, error) {
	...
	data, err := c.newClient(u).GetFile(nn[0], nn[1], b.Commit, f, t)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func (c *Coding) FileRef(u *model.User, r *model.Repo, ref, f string) ([]byte, error) {
	...
	data, err := c.newClient(u).GetFile(nn[0], nn[1], ref, f, t)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func (c *Coding) Activate(u *model.User, r *model.Repo, link string) error {
	...
	nn := strings.Split(r.FullName, "/")
	return c.newClient(u).AddWebhook(nn[0], nn[1], link, t)
}
...
func (c *Coding) Deactivate(u *model.User, r *model.Repo, link string) error {
	...
	nn := strings.Split(r.FullName, "/")
	return c.newClient(u).RemoveWebhook(nn[0], nn[1], link, t)
}
