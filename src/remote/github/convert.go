...
func convertBranch(from *github.Branch) *model.Branch {
	branch := &model.Branch{
		Name:      *from.Name,
        // Commit:    commit,
	}
...
func convertCommit(from *github.RepositoryCommit) *model.Commit {
    commit := &model.Commit{
        SHA:         *from.SHA,
        URL:         *from.Commit.URL,
        AuthorName:  *from.Commit.Author.Name,
        AuthorEmail: *from.Commit.Author.Email,
...
func convertBranchList(from []*github.Branch) []*model.Branch {
	var branches []*model.Branch
	for _, branch := range from {
		branches = append(branches, convertBranch(branch))
	}
	return branches
}
