...
import (
	...
	"github.com/coderun-top/coderun/src/remote/gitlab/client"
)
...
func (g *Gitlab) User(token string, t int) (*model.User, error) {
...
func (g *Gitlab) AccessToken(token string) (string, error) {
	// client := NewClient(g.URL, token, g.SkipVerify)
	// opts := gitlab.CreateImpersonationTokenOptions{
	// 	Name:   "coderun",
	// 	Scope:  DefaultScope,
	// }
	// auth, _, err := client.CreateImpersonationToken(1, &opts)
	// if err != nil {
	// 	return "", err
	// }
...
func (g *Gitlab) Branches(u *model.User, owner, name string) ([]*model.Branch, error) {
...
func (g *Gitlab) Branch (u *model.User, owner, name, branchName string) (*model.Branch, error) {
	client := NewClient(g.URL, u.Token, g.SkipVerify)
	newname := strings.Replace(name, "/", "%2F", -1)
...
func mergeRequest(parsed *client.HookPayload, req *http.Request) (*model.Repo, *model.Build, error) {
	...
	build.Link = obj.Url
	logrus.Debugf("build 222:%s", build)
