...
type Remote interface {
	...
	// Get Access token for access repository
	AccessToken(token string) (string, error)
	...
	Branches(u *model.User, owner, repo string) ([]*model.Branch, error)
	Branch(u *model.User, owner, repo, branchName string) (*model.Branch, error)
...
func AccessToken(c context.Context, token string) (string, error) {
	return FromContext(c).AccessToken(token)
}
...
func Branches(c context.Context, u *model.User, owner, repo string) ([]*model.Branch, error) {
	return FromContext(c).Branches(u, owner, repo)
}

func Branch(c context.Context, u *model.User, owner, repo, branchName string) (*model.Branch, error) {
	return FromContext(c).Branch(u, owner, repo, branchName)
}
