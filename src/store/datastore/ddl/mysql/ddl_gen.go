...
var migrations = []struct {
	...
	{
		name: "create-table-registry-repo",
		stmt: createTableRegistryRepo,
	},
	...
	{
		name: "create-table-pipeline_env",
		stmt: createTablePipelineEev,
	},
	{
		name: "create-table-webhook",
		stmt: createTableWebhook,
	},
	{
		name: "create-table-webhook-project",
		stmt: createTableWebhookProject,
	},
	{
		name: "create-table-keys",
		stmt: createTableKeys,
	},
	{
		name: "create-table-tags",
		stmt: createTableTags,
	},
	{
		name: "create-table-agent-repo",
		stmt: createTableAgentRepo,
	},
...
,user_token_name VARCHAR(500)
,user_provider INTEGER
,oauth BOOLEAN
,user_host VARCHAR(500)

,UNIQUE(project_name, user_token_name, oauth)
);
`
...
,UNIQUE(repo_full_name, repo_user_id)
);
`
...
,config_file_path VARCHAR(250)
,config_file INTEGER
,config_branche VARCHAR(250)
,UNIQUE(config_hash, config_repo_id, config_branche)
);
`

var createTableRegistryRepo = `
CREATE TABLE IF NOT EXISTS registry_repo (
 registry_id       INTEGER PRIMARY KEY AUTO_INCREMENT
,registry_repo_repo_id  INTEGER
,registry_repo_branche    VARCHAR(250)
,registry_repo_registry_id INTEGER
,UNIQUE(registry_repo_repo_id, registry_repo_branche)
);
`
...
var createTablePipelineEev = `
CREATE TABLE IF NOT EXISTS pipeline_env (
pipeline_env_id INTEGER PRIMARY KEY AUTO_INCREMENT
,config_id INTEGER NOT NULL
,env_key VARCHAR(250)
,env_value  VARCHAR(1000)
,UNIQUE(config_id, env_key)
);
`
var createIndexPipelineEev = `
CREATE INDEX ix_pipeline_env ON pipeline_env (pipeline_env_id);
`


var createTableWebhook = `
...
CREATE TABLE IF NOT EXISTS agent_keys (
 keys_id  INTEGER PRIMARY KEY AUTO_INCREMENT
,keyname VARCHAR(500)
,project VARCHAR(250)
,UNIQUE(project)
);
...
CREATE TABLE IF NOT EXISTS tags (
 tags_id  INTEGER PRIMARY KEY AUTO_INCREMENT
,agent_id   INTEGER
,name VARCHAR(500)
