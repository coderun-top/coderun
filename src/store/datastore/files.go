...
func (db *datastore) FileList(build *model.Build) ([]*model.File, error) {
	...
	list := []*model.File{}
	err := db.Omit("file_data").Where("file_build_id = ?",build.ID).Find(&list).Error
	return list, err
}
...
func (db *datastore) FileFind(proc *model.Proc, name string) (*model.File, error) {
	...
	// return file, err
	file := new(model.File)
	err := db.Omit("file_data").Where("file_proc_id = ? AND file_name = ?",proc.ID, name).First(&file).Error
	return file, err
}
...
func (db *datastore) FileRead(proc *model.Proc, name string) (io.ReadCloser, error) {
	...
	file := new(model.File)
	err := db.Where("file_proc_id = ? AND file_name = ?",proc.ID, name).First(&file).Error
	buf := bytes.NewBuffer(file.Data)
	return ioutil.NopCloser(buf), err
}
